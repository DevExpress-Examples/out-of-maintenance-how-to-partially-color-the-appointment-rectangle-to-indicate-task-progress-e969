...
Namespace SchedulerProject
	Partial Public Class Form1
		...
		Private Sub schedulerControl1_CustomDrawAppointmentBackground(ByVal sender As Object, ByVal e As CustomDrawObjectEventArgs) Handles schedulerControl1.CustomDrawAppointmentBackground
			...
			Dim completenessRatio As Double = 0.25
		...
		End Function
		Private Sub DrawBackGroundCore(ByVal cache As GraphicsCache, ByVal bounds As Rectangle, ByVal completenessRatio As Double)
			Dim brush1 As Brush = New SolidBrush(Color.Green)
			Dim brush2 As Brush = New SolidBrush(Color.Orange)
			cache.FillRectangle(brush1, New Rectangle(bounds.X, bounds.Y, CInt(Fix(bounds.Width * completenessRatio)), bounds.Height))
			cache.FillRectangle(brush2, New Rectangle(bounds.X + CInt(Fix(bounds.Width * completenessRatio)), bounds.Y, CInt(Fix(bounds.Width * (1 - completenessRatio))), bounds.Height))
		End Sub
